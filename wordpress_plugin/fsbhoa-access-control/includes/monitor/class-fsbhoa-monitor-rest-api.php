...
error_log('--- register_routes called ---');
    ...
    public function enrich_event_callback( WP_REST_Request $request ) {
        ...
        $cardholder_query = $wpdb->prepare( "SELECT id, first_name, last_name, photo FROM ac_cardholders WHERE rfid_id = %s", $rfid_id );
    ...
    public function log_event_callback( WP_REST_Request $request ) {
        ...
        $log_data = [
            ...
            'rfid_id'               => sprintf('%08d', absint($params['CardNumber'])),
            'event_type_code'       => absint($params['Reason']),
            'event_description'     => sanitize_text_field($params['EventMessage']),
            'access_granted'        => $params['Granted'] ? 1 : 0,
        ];

        // Try to find the cardholder_id based on the rfid_id
...
/**
    ...
    public function get_recent_activity_callback( WP_REST_Request $request ) {
        ...
        // Modified to select the binary photo data (ch.photo)
        $query = $wpdb->prepare(
            "SELECT
                l.event_timestamp,
                l.access_granted,
